...
> (Nix requires it at root) — see [Asset ownership](#asset-ownership). History: the
...
| `flake.nix`, `flake.lock`, `Makefile` | build | repo root — the flake's `src = self` packages repo-wide code and callers invoke `.#`, so `flake.nix` must root the repo; `flake.lock` + `Makefile` sit beside it |
| `nix/` (closure + image definitions) | build | `modules/build/nix/` — not referenced by `flake.nix`, so not root-bound |
