...
```bash
deploy/lambda/grab_instance.sh        # poll until a CC>=9.0 GPU is free, then launch it
deploy/lambda/setup_node.sh   <ip>    # install vLLM + deps on the box (idempotent)
deploy/lambda/start_server.sh <ip>    # resolve -> build (real nix closure if available) -> serve
deploy/lambda/verify.sh               # send identical batches twice, compare bundles -> conformant?
```

Other scripts:
- `setup.sh` — the one-shot install body (`setup_node.sh` runs it remotely; or `ssh ubuntu@<ip> 'bash -s' < deploy/lambda/setup.sh`).
...
```bash
deploy/vast/grab_cluster.sh                       # provision 4x H100 SXM -> writes cluster.env
deploy/vast/setup_cluster.sh <head> <w1> <w2> <w3>  # Ray cluster + multi-node vLLM (args are host:port)
deploy/vast/teardown_cluster.sh [contract_id ...]   # destroy nodes (no args = ALL instances — careful)
```

## `warden/` — active warden service

```bash
sudo deploy/warden/setup.sh install   # deps + config (NetfilterQueue, libnetfilter-queue)
sudo deploy/warden/setup.sh start     # add iptables NFQUEUE rules
sudo deploy/warden/setup.sh stop      # remove the rules
sudo deploy/warden/setup.sh status    # show rules + warden status
```
