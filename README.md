...
```bash
git clone https://github.com/anonymous/deterministic_serving_stack
...
The published image at `ghcr.io/anonymous/deterministic-serving-runtime:latest` predates the `/run` and `/replay` endpoints used by the audit demo and is currently private. Use `scripts/demo.sh` (above) for the reviewer flow. To run from a container, rebuild from this checkout (`nix build .#oci`) and load into Docker:
